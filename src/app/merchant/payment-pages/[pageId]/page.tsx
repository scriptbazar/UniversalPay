...
export default function PaymentPageDetailPage() {
  ...
  useEffect(() => {
    async function fetchData() {
        setLoading(true);
        const link = await getPaymentLinkBySlug(pageId);
        if (link) {
          setLinkDetails(link);
  ...
  if (loading || !linkDetails) {
    return <div>Loading...</div>; // This can be a loading state in a real app
  }

  return (
    <div className="space-y-6">
        <Link href="/merchant/payment-pages" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
                         ...
                         {transactions.length === 0 && (
                            <TableRow>
                                <TableCell colSpan={5} className="text-center h-24">
                                    No transactions found for this page yet.
                                </TableCell>
                            </TableRow>
                        )}
                    </TableBody>
            ...
            <DialogContent>
                ...
                {selectedTransaction && (
                 ...
                 <DialogFooter className="sm:justify-between gap-2">
                    ...
                    <Button asChild>
                        <Link href={`/merchant/customers`}>View Customer</Link>
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
